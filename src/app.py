...
def _bootstrap_llm_env_from_secrets() -> None:
    # Allow users to configure Gemini key/model in .streamlit/secrets.toml.
...
def seed_demo_corrections(cases_df: pd.DataFrame) -> int:
    ...
    if not existing.empty and "reviewer_name" in existing.columns:
        already_seeded = int(
            (existing["reviewer_name"] == "demo-seed").sum()
        )
...
def show_metrics(reviews_df: pd.DataFrame) -> None:
    ...
    if reviews_df.empty:
        st.info(
            "No review records yet. Submit at least one decision to populate metrics."
        )
        return
    ...
    accepted = int(
        (reviews_df["reviewer_decision"] == "Accepted").sum()
    )

    edited = int(
        (reviews_df["reviewer_decision"] == "Edited").sum()
    )

    rejected = int(
        (reviews_df["reviewer_decision"] == "Rejected").sum()
    )

    agreement_rate = (
        float(reviews_df["agreement_flag"].mean())
        if total
        else 0.0
    )
    ...
    overrides = reviews_df[
        reviews_df["reviewer_decision"].isin(
            ["Edited", "Rejected"]
        )
    ].copy()
    ...
    if overrides.empty:
        ...
        st.dataframe(
            overrides[display_cols].tail(10),
...
st.caption(
    "Hybrid deterministic + LLM diagnostics with mandatory Human-in-the-Loop review"
)
...
with st.sidebar:
    ...
    if st.button("Seed 5 demo correction logs"):
        ...
        if inserted_count:
            st.success(
                f"Inserted {inserted_count} seeded correction records."
            )

        else:
            st.info(
                "Seed records already exist."
            )
...
c1.markdown(
    f"**Concept**: {selected_case['concept_tag']}"
)

c2.markdown(
    f"**Severity**: {selected_case['severity']}"
)

c3.markdown(
    f"**Expected OSI Layer**: {selected_case['osi_layer']}"
)
...
if st.button(
    ...
    with st.spinner(
        "Running checker and Gemini diagnosis..."
    ):
...
if "diagnosis" in st.session_state:
    ...
    with left:
        ...
        st.write(
            diagnosis["checker"]["summary"]
        )

        findings_df = pd.DataFrame(
            diagnosis["checker"]["findings"]
        )

        if findings_df.empty:

            st.caption(
                "No rule match. LLM inference required."
            )
    ...
    reviewer_reason = st.text_area(
        ...
        placeholder=(
            "Add why you accepted, edited, "
            "or rejected this diagnosis."
        ),
    )
    ...
    reviewer_fix = st.text_area(
        ...
        placeholder=(
            "If Edited, provide corrected fix commands. "
            "If Accepted, you can keep AI fix."
        ),
        value="\n".join(
            diagnosis["final"].get(
                "fix_steps",
    ...
    if (
        decision == "Edited"
        and not reviewer_fix.strip()
    ):

        st.error(
            "Edited decisions require corrected fix steps."
        )
    ...
    if st.button(
        ...
        review_record = {
            ...
            "ai_root_cause": diagnosis["final"].get(
                "root_cause",
                "",
            ),

            "ai_confidence": diagnosis["final"].get(
                "confidence",
                0.0,
            ),
            ...
            "agreement_flag": _decision_to_agreement(
                decision
            ),
        }
        ...
        st.success(
            "Review decision saved to CSV and markdown audit log."
        )
